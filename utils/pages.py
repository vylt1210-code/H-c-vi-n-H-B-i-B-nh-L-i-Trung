...
from .constants import DAYS, CAS, ATTENDANCE_COLS
...
def today_timeline(df, busy_slots):
    ...
    for ca, time_range in CAS.items():
        ...
        display_time = time_range.replace(" - ", "<br>")
        html += (
            f'<div class="timeline-row">'
            f'<div class="timeline-time">{display_time}</div>'
            f'<div>'
            f'<div class="timeline-main">{ca}</div>'
            f'<div class="timeline-sub">{count} học viên</div>'
            f'</div>'
...
def page_dashboard(df, busy_slots):
    ...
    for icon, text in quick_items:
        st.markdown(
            f'<div class="quick-card"><div class="quick-icon">{icon}</div><div class="quick-text">{text}</div></div>',
            unsafe_allow_html=True,
        )
    st.markdown("</div>", unsafe_allow_html=True)
        ...
        with st.form("edit_student"):
            ...
            with c1:
                ...
                ecourse = st.selectbox(
                    "Khóa",
                    options,
                    index=options.index(old_course) if old_course in options else 3,
                )
