...
DATA_FILE = DATA_DIR / "hoc_vien.csv"
BUSY_FILE = DATA_DIR / "lich_ban.csv"
TEACHER_FEE_FILE = DATA_DIR / "hoc_phi_thay.csv"

# Tương thích nếu file cũ nằm ngoài thư mục data/
OLD_DATA_FILE = BASE / "hoc_vien.csv"
OLD_BUSY_FILE = BASE / "lich_ban.csv"
OLD_TEACHER_FILE = BASE / "hoc_phi_thay.csv"

if not DATA_FILE.exists() and OLD_DATA_FILE.exists():
    DATA_FILE = OLD_DATA_FILE
if not BUSY_FILE.exists() and OLD_BUSY_FILE.exists():
    BUSY_FILE = OLD_BUSY_FILE
if not TEACHER_FEE_FILE.exists() and OLD_TEACHER_FILE.exists():
    TEACHER_FEE_FILE = OLD_TEACHER_FILE
...
def load_teacher_fees():
    if TEACHER_FEE_FILE.exists():
        df = pd.read_csv(TEACHER_FEE_FILE)
    else:
        df = pd.DataFrame([
...
def save_teacher_fees(df):
    df.to_csv(TEACHER_FEE_FILE, index=False, encoding="utf-8-sig")
...
def css():
    ...
    .summary-card.green { border-left: 6px solid #2dc653; }
    .summary-card.orange { border-left: 6px solid #f59f00; }
    .summary-card.red { border-left: 6px solid #e63946; }
    .summary-card.blue { border-left: 6px solid #00b4d8; }
    .summary-card.purple { border-left: 6px solid #7b2cbf; }
    .summary-card.teal { border-left: 6px solid #008f8f; }
    ...
    .report-card {
        ...
        border-radius: 24px;
        padding: 20px;
        box-shadow: 0 12px 30px rgba(2,62,138,.08);
        margin-bottom: 18px;
    }
    .report-title {
        color: #023e8a;
    ...
    .bar-fill {
        height: 12px;
        background: linear-gradient(90deg,#00b4d8,#2dc653);
        border-radius: 999px;
    }
...
def dashboard_summary_card(label, value, icon, note="", color="blue"):
    st.markdown(
        f"""
...
def get_active_students(df):
    if df.empty or "Trạng thái khóa" not in df.columns:
        return df
...
def next_or_current_slot_for_today(busy_slots):
    ...
    for ca, time_range in CAS.items():
        ...
        if start <= now_time < end:
            current_slot = (ca, time_range)
            break

        if now_time < start and next_slot is None:
            next_slot = (ca, time_range)

    return current_slot, next_slot


def dashboard_today_timeline(df, busy_slots):
    today = current_vn_day()
    html = """
    <div class="report-card">
        <div class="report-title">📅 Lịch dạy hôm nay</div>
    """
...
def dashboard_action_panel(df, busy_slots):
    active_df = get_active_students(df)
    almost_df = active_df[(active_df["Tổng buổi"] - active_df["Đã học"]) <= 2].copy() if not active_df.empty else active_df
    unpaid_count = len(active_df[active_df["Đã nhận tiền dạy"] == "Chưa"]) if not active_df.empty else 0
    teacher_debt = teacher_remaining_total()
    current_slot, next_slot = next_or_current_slot_for_today(busy_slots)

    current_text = "Không có ca đang diễn ra"
    if current_slot:
        current_text = f"{current_slot[0]} • {current_slot[1]}"
    elif next_slot:
        current_text = f"Ca tiếp theo: {next_slot[0]} • {next_slot[1]}"

    st.markdown(
        f"""
        <div class="smart-panel">
            <div class="smart-title">🔔 Thông báo thông minh</div>
            <div class="smart-item">🕒 {datetime.now(VN_TZ).strftime('%H:%M:%S')} GMT+7 — {current_text}</div>
            <div class="smart-item">⚠️ {len(almost_df)} học viên sắp hết khóa trong 1–2 buổi.</div>
            <div class="smart-item">💰 {unpaid_count} học viên chưa nhận tiền dạy.</div>
...
def dashboard_near_finish_table(df):
    active_df = get_active_students(df)
    if active_df.empty:
...
def dashboard_teacher_report():
    try:
        ...
        st.info("Chưa có dữ liệu tiền giáo viên.")
...
def page_dashboard(df, busy_slots):
    ...
    hero("🏊 BLT Swimming Club", f"Trung tâm điều hành • {now.strftime('%H:%M:%S')} GMT+7 • {now.strftime('%d/%m/%Y')}")

    active_df = get_active_students(df)
    finished_df = get_finished_students(df)

    today = current_vn_day()
    today_students = sum(len(get_students_by_slot(df, today, ca)) for ca in CAS)
    almost_done = len(active_df[(active_df["Tổng buổi"] - active_df["Đã học"]) <= 2]) if not active_df.empty else 0
    unpaid = len(active_df[active_df["Đã nhận tiền dạy"] == "Chưa"]) if not active_df.empty else 0
    teacher_debt = teacher_remaining_total()
    ...
    with c1:
        dashboard_summary_card("Đang học", len(active_df), "👨‍🎓", "Học viên còn trong lịch", "blue")
    with c2:
        dashboard_summary_card("Kết thúc khóa", len(finished_df), "🎓", "Không còn hiện trong lịch", "green")
    with c3:
        dashboard_summary_card("Học viên hôm nay", today_students, "📅", f"{today}", "teal")

    c4, c5, c6 = st.columns(3)
    with c4:
        dashboard_summary_card("Sắp hết buổi", almost_done, "⚠️", "Còn 1–2 buổi", "orange")
    with c5:
        dashboard_summary_card("Chưa nhận tiền", unpaid, "💰", "Cần kiểm tra học phí", "red")
    with c6:
        dashboard_summary_card("Tiền GV còn", teacher_debt, "👨‍🏫", "Số HV chưa thanh toán GV", "purple")

    st.markdown('</div>', unsafe_allow_html=True)

    dashboard_action_panel(df, busy_slots)

    left, right = st.columns([1.25, .85])
    with left:
        dashboard_today_timeline(df, busy_slots)
    with right:
        dashboard_near_finish_table(df)
        dashboard_teacher_report()
