...
TEACHER_FEE_FILE = resolve_file("hoc_phi_thay.csv")
...
def load_teacher_fees() -> pd.DataFrame:
    columns = ["Thầy", "Tổng học viên", "Đã thanh toán"]
    if TEACHER_FEE_FILE.exists():
        tdf = pd.read_csv(TEACHER_FEE_FILE)
    else:
        tdf = pd.DataFrame([
    ...
    for col in columns:
        if col not in tdf.columns:
            tdf[col] = 0 if col != "Thầy" else ""

    for teacher in ["Đạt", "Long"]:
        if teacher not in tdf["Thầy"].astype(str).tolist():
    ...
    tdf["Tổng học viên"] = pd.to_numeric(tdf["Tổng học viên"], errors="coerce").fillna(0).astype(int)
    tdf["Đã thanh toán"] = pd.to_numeric(tdf["Đã thanh toán"], errors="coerce").fillna(0).astype(int)
    tdf.loc[tdf["Đã thanh toán"] > tdf["Tổng học viên"], "Đã thanh toán"] = tdf["Tổng học viên"]
    return tdf[columns]


def save_teacher_fees(tdf: pd.DataFrame):
    tdf.to_csv(TEACHER_FEE_FILE, index=False, encoding="utf-8-sig")
...
def image_banner(filename, caption=None):
    path = ASSETS / filename
    if path.exists():
        st.image(str(path), use_container_width=True, caption=caption)
    else:
        st.warning(f"Không tìm thấy ảnh: assets/{filename}")
...
def page_teacher_fee():
    hero("👨‍🏫 Học phí thầy", "Theo dõi số học viên và số đã thanh toán cho thầy Đạt, Long")
    tdf = load_teacher_fees()

    st.info("Ví dụ: tổng 10 học viên, đã thanh toán 9 thì còn 1. Khi có học viên mới bấm +1 học viên. Khi trả tiền bấm +1 đã thanh toán.")

    for teacher in ["Đạt", "Long"]:
        idx_list = tdf[tdf["Thầy"] == teacher].index
        if len(idx_list) == 0:
            continue
        ...
        <div class='student-card'>
            ...
            <div class='progress-wrap'><div class='progress-bar' style='width:{0 if total == 0 else int(paid / max(total, 1) * 100)}%'></div></div>
        ...
        with c1:
            if st.button(f"➕ Thêm 1 học viên - {teacher}", key=f"add_teacher_student_{teacher}", use_container_width=True):
                tdf.loc[idx, "Tổng học viên"] = total + 1
                save_teacher_fees(tdf)
                st.success(f"Đã cập nhật thầy {teacher}: tổng học viên +1.")
                st.rerun()

        with c2:
            disabled_paid = paid >= total
            if st.button(f"💵 Thanh toán 1 HV - {teacher}", key=f"pay_teacher_student_{teacher}", use_container_width=True, disabled=disabled_paid):
                tdf.loc[idx, "Đã thanh toán"] = min(paid + 1, total)
                save_teacher_fees(tdf)
                st.success(f"Đã cập nhật thầy {teacher}: đã thanh toán +1.")
                st.rerun()

        with c3:
            if st.button(f"↩️ Trừ 1 thanh toán - {teacher}", key=f"undo_teacher_pay_{teacher}", use_container_width=True, disabled=paid <= 0):
                tdf.loc[idx, "Đã thanh toán"] = max(paid - 1, 0)
                save_teacher_fees(tdf)
                st.success(f"Đã trừ 1 thanh toán của thầy {teacher}.")
                st.rerun()

        with st.expander(f"✏️ Chỉnh tay số liệu thầy {teacher}"):
            new_total = st.number_input(f"Tổng học viên - {teacher}", min_value=0, value=total, key=f"manual_total_{teacher}")
            new_paid = st.number_input(f"Đã thanh toán - {teacher}", min_value=0, max_value=int(new_total), value=min(paid, int(new_total)), key=f"manual_paid_{teacher}")

            if st.button(f"💾 Lưu số liệu thầy {teacher}", key=f"save_manual_{teacher}", use_container_width=True):
                tdf.loc[idx, "Tổng học viên"] = int(new_total)
                tdf.loc[idx, "Đã thanh toán"] = int(new_paid)
                save_teacher_fees(tdf)
                st.success(f"Đã lưu số liệu thầy {teacher}.")
                st.rerun()
...
def main():
    ...
    with st.sidebar:
        ...
        page = st.radio("Điều hướng", ["🏠 Dashboard", "📅 Lịch dạy", "👨‍🎓 Học viên", "💰 Học phí", "👨‍🏫 Tiền thầy", "⚙️ Cài đặt"], label_visibility="collapsed")
    ...
    elif page == "👨‍🏫 Tiền thầy": page_teacher_fee()
