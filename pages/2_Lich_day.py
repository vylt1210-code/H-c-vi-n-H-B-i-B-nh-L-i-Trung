...
st_autorefresh(interval=60_000, key="lich_day_refresh")

hero("📅 Lịch dạy", "Lịch tự cập nhật theo thời gian thực theo giờ Việt Nam")
...
now_text = datetime.now(VN_TZ).strftime("%H:%M:%S")
date_text = datetime.now(VN_TZ).strftime("%d/%m/%Y")

st.info(f"🕒 Thời gian Việt Nam: {now_text} — {date_text} — app tự cập nhật mỗi 60 giây")
